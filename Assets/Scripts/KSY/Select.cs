...
public class Select : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // 출구 좌표 추가
        Exit.Add(new Vector3(-43.11f, 14.61f, 0));
        Exit.Add(new Vector3(-65.39f, 14.69f, 0));
        Exit.Add(new Vector3(-95.73f, 4.26f, 0));

        Boss = new Vector3(-127.8f, -3.2f, 0);
    ...
    public void Update()
    {
        if (Exit.Count != 0)
        {
            if (Vector3.Distance(currentCharacter.transform.position, Exit[Draw]) <= 4)
            {
                Exit.RemoveAt(Draw);
                SpawnCharacter(0);
            }
